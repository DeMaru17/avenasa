...
<div class="inline-flex items-center bg-slate-100 rounded-full p-1 gap-0.5 {{ $class }}" role="group" aria-label="{{ __('Select Language') }}">
    <a
        ...
        onclick="if(window.ANSAnalytics) window.ANSAnalytics.trackLanguageSwitch({ sourceLocale: '{{ $currentLocale }}', targetLocale: 'id', currentPath: window.location.pathname });"
    ...
    <a
        ...
        onclick="if(window.ANSAnalytics) window.ANSAnalytics.trackLanguageSwitch({ sourceLocale: '{{ $currentLocale }}', targetLocale: 'en', currentPath: window.location.pathname });"
